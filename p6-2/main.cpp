...
private:
	...
	Program gouraud, constant, phong, cook, ash;
	Cylinder cone;
	Sphere sph;
	Box box;
	Rect suelo;
	...
	std::shared_ptr<FloatSliderWidget> m, lambda, nu, nv, spec_coeff, diff_coeff;
	//std::shared_ptr<CheckBoxWidget> phongShading;
	//bool phongGui;
	//GLint phongPass;
};
...
void MyRender::setup() {
	...
	cook.addAttributeLocation(Mesh::VERTICES, "position");
	cook.addAttributeLocation(Mesh::NORMALS, "normal");
	ash.addAttributeLocation(Mesh::VERTICES, "position");
	ash.addAttributeLocation(Mesh::NORMALS, "normal");
	...
	//GLint compileBlockIndex = glGetUniformBlockIndex(gouraud, mats);
	phong.connectUniformBlock(mats, UBO_GL_MATRICES_BINDING_INDEX);
	cook.connectUniformBlock(mats, UBO_GL_MATRICES_BINDING_INDEX);
	ash.connectUniformBlock(mats, UBO_GL_MATRICES_BINDING_INDEX);
	...
	cook.connectUniformBlock(luces, UBO_LIGHTS_BINDING_INDEX);
	ash.connectUniformBlock(luces, UBO_LIGHTS_BINDING_INDEX);
	...
	cook.connectUniformBlock(UBOMaterial::build(), UBO_MATERIALS_BINDING_INDEX);
	ash.connectUniformBlock(UBOMaterial::build(), UBO_MATERIALS_BINDING_INDEX);

	// Este shader se encarga de calcular la iluminación, usando
	// el algoritmo de Gouraud 
	gouraud.loadFiles("../ej6-2/gouraud");
	gouraud.compile();
	phong.loadFiles("../ej6-2/phong");
	phong.compile();
	cook.loadFiles("../ej6-2/cook");
	cook.compile();
	ash.loadFiles("../ej6-2/ash");
	ash.compile();

	//phongGui = false;
	//phongPass = gouraud.getUniformLocation("phongFlag");//it gives -1
	//phongPass = glGetUniformLocation(gouraud.getId(), "phongFlag");//it gives -1
	...
	//phongPass = gouraud.getUniformLocation("phongFlag");
...
void MyRender::render() {
	...
	//if (!phongShading->get()) gouraud.use();
	//if (phongShading->get()) phong.use();
	...
	case 2:
		cook.use();
		glUniform1f(21, m->get());
		glUniform1f(22, lambda->get());
		break;
	case 3:
		ash.use();
		glUniform1f(25, nu->get());
		glUniform1f(26, nv->get());
		glUniform1f(27, spec_coeff->get());
		glUniform1f(28, diff_coeff->get());
		break;
	default:
		gouraud.use();
		break;
	...
	//phongGui = phongShading->get();
	//glUniform1f(phongPass, (phongGui));
	//glUniform1f(19, phongGui);
	...
	suelo.render();
	mats->translate(GLMatrices::MODEL_MATRIX, 0.0, 0.0, -0.001f);
	mats->scale(GLMatrices::MODEL_MATRIX, glm::vec3(1.0, 1.0, -1.0));
	suelo.render();
	mats->popMatrix(GLMatrices::MODEL_MATRIX);
...
void MyRender::buildGUI() {
	...
	panel->setSize(280, 750);

	//phongShading = std::make_shared<CheckBoxWidget>("Use Phong shading", false);
	//panel->addWidget(phongShading);
	shaderType = std::make_shared<ListBoxWidget<>>("light formula", std::vector<std::string> {
		"Vertex", "Blinn-Phong", "Cook-Torrance", "Ashkihmin"}, 0);
	panel->addWidget(shaderType);

	panel->addWidget(std::make_shared<Label>("Cook Coeficients"));
	lambda = std::make_shared<FloatSliderWidget>("lambda", 0.75, 0, 2);
	panel->addWidget(lambda);
	m = std::make_shared<FloatSliderWidget>("m", 0.3, 0, 2);
	panel->addWidget(m);

	panel->addWidget(std::make_shared<Label>("Ash Coeficients"));
	nu = std::make_shared<FloatSliderWidget>("nu", 10, 0, 20);
	panel->addWidget(nu);
	nv = std::make_shared<FloatSliderWidget>("nv", 100, 0, 200);
	panel->addWidget(nv);
	spec_coeff = std::make_shared<FloatSliderWidget>("spec_coeff", 0.5, 0, 2);
	panel->addWidget(spec_coeff);
	diff_coeff = std::make_shared<FloatSliderWidget>("diff_coeff", 0.5, 0, 2);
	panel->addWidget(diff_coeff);
