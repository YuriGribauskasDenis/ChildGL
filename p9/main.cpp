...
void MyRender::setup() {
	...
	plane.accept([material](Mesh& m) { m.setTangent(vec3(1.0f, 0.0f, 0.0f)); });
...
void MyRender::buildGUI() {
	...
	depthSize = std::make_shared<FloatSliderWidget>("Depth", 0.1f, 0.001f, 0.5f, ashader, "depth");
	...
	panel->addWidget(std::make_shared<Label>("Parrallax type"));
	fragmentType = std::make_shared<ListBoxWidget<>>("PP", std::vector<std::string> {
		"lectures", "Fog", "Sun", "light"}, 0, ashader, "PP");
