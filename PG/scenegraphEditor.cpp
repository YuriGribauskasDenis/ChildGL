...
bool nodePathExists(NodePath nodePath, std::shared_ptr<Node> root) {
	size_t nodePathIdx;
	for (nodePathIdx = 0; nodePathIdx < nodePath.size(); nodePathIdx++) {
		std::shared_ptr<Group> g = std::dynamic_pointer_cast<Group>(root);
		// ignore the dummy nodes in the tree
		...
		if (!g)
			return (nodePathIdx == nodePath.size() - 1) &&
			(nodePath.back().get() == root.get());
		if (g->containsChild(nodePath[nodePathIdx])) {
			root = nodePath[nodePathIdx];
		}
		else {
			return false;
		}
	}
	return nodePathIdx == nodePath.size();
}
