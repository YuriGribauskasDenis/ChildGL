...
void AssimpWrapper::loadMaterials() {
	...
	for (unsigned int i = 0; i < scene->mNumMaterials; ++i) {
		...
		loadTextures(scene->mMaterials[i], *theMaterial);
		INFO(printMaterialInfo(mtl) + " TextureCount: " + std::bitset<32>(theMaterial->getTextureCounters()).to_string());
		result->addMaterial(theMaterial);
	}
}
