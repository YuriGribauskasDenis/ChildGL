...
public:
  MyRender()
    ...
    luz(.02f, 5, 5, vec4(1.0f, 1.0f, 1.0f, 1.0f)),
    luz2(.09f, 5, 5, vec4(0.0f, 1.0f, 1.0f, 1.0f)),
    zbuffer(2, 2),
    zbuffer2(2, 2),
    ci(vec4(0.0, 2.0, 0.0, 1.0),vec4(3.0, 2.0, 0.0, 1.0),vec3(0.0, 1.0, 0.0), 3.6f),
    ci2(vec4(0.0, 2.0, 0.0, 1.0),vec4(3.0, 2.0, 0.0, 1.0),vec3(0.0, 1.0, 0.0), 7.2f)
  {};
...
private:
  ...
  FBO fbo, fbo2;
  std::shared_ptr<Texture2D> depthTexture, depthTexture2;
...
void MyRender::prepareFBO(uint width, uint height) {
  ...
  if (!fbo.isComplete()||!fbo2.isComplete())
    ERRT("FBO incompleto, lo siento");
}
...
void MyRender::setup()
{
  ...
  shadowShader.loadFiles("../p10/shadowMap");
  ...
  zshader->loadFiles("../p10/zbuffershow");
  zshader->compile();
...
void MyRender::buildGUI() {
  ...
  fixAS = std::make_shared<CheckBoxWidget>("AutoShadow", false);
  panel->addWidget(fixAS);
